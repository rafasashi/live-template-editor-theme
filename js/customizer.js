jQuery( document ).ready(function($) {
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					ltpleAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				ltpleGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			ltpleGetAllInputs($(this).parent().parent().parent());
		}
	});
	...
	function ltpleAppendRow($element, defaultValue = '') {
	...
	function ltpleGetAllInputs($element) {
	...
	$('.multi-image-checkbox').on('change', function () {
	  ltpleGetAllImageCheckboxes($(this).parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function ltpleGetAllImageCheckboxes($element) {
	...
	$( ".pill_checkbox_control .sortable" ).sortable({
		...
		update: function( event, ui ) {
			ltpleGetAllPillCheckboxes($(this).parent());
		}
	});

	$('.pill_checkbox_control .sortable-pill-checkbox').on('change', function () {
		ltpleGetAllPillCheckboxes($(this).parent().parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function ltpleGetAllPillCheckboxes($element) {
	...
	$('.google-fonts-list').on('change', function() {
		...
		var indexes = $.map(bodyfontcontrol.ltplefontslist, function(obj, index) {
		...
		$.each(bodyfontcontrol.ltplefontslist[index].variants, function(val, text) {
		...
		$(this).parent().parent().find('.google-fonts-category').val(bodyfontcontrol.ltplefontslist[index].category);

		ltpleGetAllSelects($(this).parent().parent());
	});
	...
	function ltpleGetAllSelects($element) {
		var selectedFont = {
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].ltpletinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].ltpletinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].ltplemediabuttons;
 ...
 */
( function( $, api ) {
	api.sectionConstructor['ltple-upsell'] = api.Section.extend( {
